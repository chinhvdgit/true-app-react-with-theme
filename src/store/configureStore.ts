...
export default function configureStore(history: History, initialState?: IApplicationState) {
    ...
    const applyMW = applyMiddleware(thunk, routerMiddleware(history));

    const storeEnhancer = devToolsExtension ? devToolsExtension() : <S>(next: StoreEnhancerStoreCreator<S>) => next;
